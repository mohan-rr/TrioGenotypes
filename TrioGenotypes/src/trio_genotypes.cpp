...
void open_bed_file(std::string &bed_filename, std::ifstream& bed_file, int n_ind, int n_snp){
	...
	bed_file.seekg(0, bed_file.end);
    int length = bed_file.tellg();
    bed_file.seekg(0, bed_file.beg);
    if (length != ((n_ind+3)/4)*n_snp + 3){
        Rcpp::Rcout << "file size: " << length << " expected: " << ((n_ind+3)/4)*n_snp + 3 << "\n";
		Rcpp::stop(".bed file size not expected length");
    } 
