...
note that sometimes the frequency is "reversed" (i.e.: 222 more common than 000) this would be because the A1 allele in the plink file is actually the major allele.  

## To-do:

* Right now the function only counts case and control trios. Can add counts for dyads/parents/singletons later.
* Function currently reads in all SNPs in the .bim/.bed file. Should add a way to select a range/subset of SNPs.
...
* can multithread and do in batches.
* should benchmark use of Rcpp::IntegerMatrix compared to creating a matrix in R and using the SEXP pointer.
* can allow the .bed/.bim/.fam files to be specified separately
* should not return a ctrl/case matrix if there are no case/ctrl trios
